...
CREATE TABLE staff (
  ...
  standard_period_minutes INTEGER CHECK (
    standard_period_minutes IS NULL OR standard_period_minutes > 0
  ),
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
  updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
) STRICT;
...
CREATE TABLE schedule_versions (
  ...
  effective_to TEXT CHECK (
    ...
    (
      effective_to GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' AND
...
CREATE TABLE schedule_entries (
  ...
  start_time TEXT NOT NULL CHECK (
    ...
    substr(start_time, 1, 2) <= '23'
  ),
  end_time TEXT NOT NULL CHECK (
    end_time GLOB '[0-2][0-9]:[0-5][0-9]' AND
    substr(end_time, 1, 2) <= '23' AND
    start_time < end_time
  ),
  activity_type TEXT NOT NULL CHECK (
    activity_type IN (
      'instruction', 'plan', 'admin', 'lunch', 'duty', 'after_school',
      'off_site', 'other'
    )
  ),
  category TEXT NOT NULL CHECK (
    category IN (
      'PRI', 'EL', 'INT', 'MS', 'HS', 'PLAN_ADMIN', 'LUNCH',
      'AFTER_SCHOOL_OTHER'
    )
  ),
...
CREATE TABLE special_schedules (
  id TEXT PRIMARY KEY,
  date TEXT NOT NULL UNIQUE CHECK (
...
CREATE INDEX special_schedules_date_status
  ON special_schedules (date, status);
...
CREATE INDEX absences_staff_dates
  ON absences (staff_id, start_date, end_date);
...
CREATE TABLE daily_sub_plans (
  ...
  finalized_at TEXT,
  structured_revision INTEGER NOT NULL DEFAULT 0 CHECK (
...
CREATE INDEX default_sub_plans_lookup
  ON default_sub_plans (absent_staff_id, day_type, status);
...
CREATE TABLE assignments (
  ...
  responsibility_type TEXT NOT NULL CHECK (
    responsibility_type IN ('instruction', 'duty', 'after_school', 'other')
  ),
  description TEXT NOT NULL,
  room_id TEXT REFERENCES rooms(id),
  ...
  resolution_type TEXT CHECK (
    ...
    resolution_type IN (
      'teacher_cover', 'redistribution', 'switch_groups', 'combine_class',
      'move_room', 'duty_coverage', 'intentional_uncovered', 'manual_override',
      'split_coverage', 'solo_coverage'
    )
  ),
  resolution_details_json TEXT CHECK (
    resolution_details_json IS NULL OR json_valid(resolution_details_json)
  ),
  status TEXT NOT NULL DEFAULT 'unresolved' CHECK (
    status IN ('unresolved', 'assigned', 'intentionally_uncovered')
  ),
  ...
  counts_toward_workload INTEGER NOT NULL DEFAULT 1 CHECK (
    counts_toward_workload IN (0, 1)
  ),
  CHECK (
    ((source_schedule_entry_id IS NOT NULL) <> (source_special_schedule_entry_id IS NOT NULL))
  ),
  CHECK (
    (
      override_acknowledged_at IS NULL AND
      override_acknowledged_by IS NULL
    ) OR (
      override_acknowledged_at IS NOT NULL AND
      override_acknowledged_by IS NOT NULL
    )
  )
) STRICT;

CREATE INDEX assignments_plan_status
  ON assignments (daily_sub_plan_id, status, start_time);
CREATE INDEX assignments_absence ON assignments (absence_id);
CREATE INDEX assignments_staff_workload
  ON assignments (assigned_staff_id, status, daily_sub_plan_id);
...
CREATE INDEX assignments_shared_responsibility
  ON assignments (daily_sub_plan_id, shared_responsibility_key, status);
...
CREATE INDEX assignment_segments_staff_time
  ON assignment_segments (staff_id, start_time, end_time);
...
CREATE TABLE generated_messages (
  ...
  generated_html TEXT,
  edited_html TEXT,
  source_plan_revision INTEGER CHECK (
    source_plan_revision IS NULL OR source_plan_revision >= 0
  )
) STRICT;

CREATE INDEX generated_messages_plan_time
  ON generated_messages (daily_sub_plan_id, generated_at DESC);
...
CREATE TABLE application_settings (
  ...
  workload_warning_threshold REAL NOT NULL DEFAULT 5.0 CHECK (
    workload_warning_threshold > 0
  ),
  workload_window_days INTEGER NOT NULL DEFAULT 7 CHECK (
    workload_window_days > 0
  ),
  split_snap_minutes INTEGER NOT NULL DEFAULT 10 CHECK (
    split_snap_minutes > 0
  ),
...
CREATE TABLE schedule_imports (
  id TEXT PRIMARY KEY,
  import_kind TEXT NOT NULL DEFAULT 'normal' CHECK (
    import_kind IN ('normal', 'special')
  ),
  schedule_name TEXT NOT NULL,
  source_file_name TEXT NOT NULL,
  source_file_sha256 TEXT NOT NULL,
  status TEXT NOT NULL CHECK (
    status IN ('staged', 'ready', 'activated', 'failed')
  ),
  ...
  sheet_name TEXT,
  recognized_staff_count INTEGER NOT NULL DEFAULT 0 CHECK (
    recognized_staff_count >= 0
  ),
  recognized_room_count INTEGER NOT NULL DEFAULT 0 CHECK (
  ...
  CHECK (
    (
      import_kind = 'normal' AND
      effective_from IS NOT NULL AND
      special_date IS NULL
    ) OR (
      import_kind = 'special' AND
      effective_from IS NULL AND
      effective_to IS NULL AND
      special_date IS NOT NULL
    )
  ),
  CHECK (
    ((activated_schedule_version_id IS NOT NULL) <> (activated_special_schedule_id IS NOT NULL)) OR
    status <> 'activated'
  )
) STRICT;
...
CREATE TABLE schedule_import_staff (
  ...
  mapping_status TEXT NOT NULL CHECK (
    mapping_status IN ('unmapped', 'exact', 'mapped', 'created')
  ),
...
CREATE TABLE school_calendar_dates (
  ...
  expected_day_type TEXT CHECK (
    expected_day_type IS NULL OR expected_day_type IN ('A', 'B')
  ),
  is_school_day INTEGER NOT NULL DEFAULT 1 CHECK (is_school_day IN (0, 1)),
  is_blackout_day INTEGER NOT NULL DEFAULT 0 CHECK (
    is_blackout_day IN (0, 1)
  ),
  expects_special_schedule INTEGER NOT NULL DEFAULT 0 CHECK (
    expects_special_schedule IN (0, 1)
  ),
  label TEXT,
  source_type TEXT NOT NULL DEFAULT 'manual_import',
  imported_by TEXT REFERENCES authorized_users(id),
  imported_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
  updated_by TEXT REFERENCES authorized_users(id),
  updated_at TEXT,
  CHECK (is_school_day = 1 OR expected_day_type IS NULL)
) STRICT;

CREATE INDEX school_calendar_dates_school_day
  ON school_calendar_dates (is_school_day, date);
CREATE INDEX school_calendar_dates_special_expectation
  ON school_calendar_dates (expects_special_schedule, date);

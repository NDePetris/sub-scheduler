...
A candidate is automatically available when a PLAN or Admin block covers the relevant time, or when the person has an applicable schedule for the day with no responsibility overlapping the Assignment. The latter source is labeled **Available**. An active, sub-eligible designated School Sub is automatically available without schedule data; absence and overlapping direct or split coverage still make the School Sub unavailable. Other overlapping activities do not create availability. Existing coverage, absence, and schedule conflicts become explainable warnings or unavailability signals. Administrators may explicitly override warnings.
...
The Resolve Sub Need response combines Assignment context, default action, availability/conflicts, current and projected workload, and permitted alternate actions. Resolution types include direct teacher cover, redistribution, switch/combine, room move, duty coverage, intentional uncovered, manual override, and split coverage as supported by the Default Sub Plan action model.

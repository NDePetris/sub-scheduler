...
In September 2026, before any database contained retained operational data, the historical `0001`–`0012` chain was intentionally retired after upgrade testing exposed data-preservation defects in table rebuilds. `migrations/0001_initial_schema.sql` now directly creates the complete intended schema and is the authoritative schema origin. Databases created from the retired chain are not supported upgrade sources. Future migrations are forward-only from this baseline, and the baseline becomes immutable once deployed.
...
Reporting uses a dedicated read-only `ReportingRepository` and a pure projection module rather than planning-state transitions. The teacher-performance summary and `GET /api/reports/teacher-performance/:staffId` detail endpoint read current Teacher identities, authoritative Absences, calendar metadata, and finalized workload-eligible Assignment/segment intervals. Missing weekday calendar metadata retains the Monday-Friday fallback but is returned as provisional coverage metadata; it is not treated as fully authoritative calendar data. Coverage Minutes are unique clock minutes per Teacher/date; Coverage Segments count every persisted direct or split coverage contribution. Detail derives coverage period equivalents from each plan's pinned schedule context and derives Plan Periods Lost with the existing PLAN-overlap rules. Configured `staff.standard_period_minutes` remains mutable current Staff state, so it can change historical equivalent/burden values; otherwise inference prefers the pinned normal Schedule Version and uses a Special Schedule only as fallback.
...
Store both the most recently generated text and the independently editable text. Editing does not update Assignments or the structured-plan revision. **Regenerate** renders from current structured data, records that source revision, and replaces the editable draft after an explicit user action. Review automatically refreshes a stale unedited draft; a stale manually edited draft remains intact and must be explicitly regenerated before finalization. **Copy to Clipboard** is a browser action; no email or messaging integration exists in MVP.
...
- The canonical D1 baseline directly creates the relational core described above plus `authorized_users`, current import/calendar/provenance fields, structured message revisions, and shared-responsibility fields. `application_settings` is a constrained single school row for MVP. Stable text IDs keep fixtures deterministic and do not make imported display names identities.
...
- Local and test identity is supplied only through server bindings and must still match an active `authorized_users` row. Production reads only `Cf-Access-Jwt-Assertion`; `jose` validates its RS256 signature through the configured Access team's rotating JWKS, issuer, Application Audience (AUD) tag, expiry, and email claim before applying the same active-administrator allowlist. `DEV_USER_EMAIL` and email headers are never used in production. Missing Access configuration is a clear server configuration error, while missing or invalid assertions return authentication errors.
...
- A Daily Sub Plan requires at least one schedule source and may pin both the applicable normal Schedule Version and an authoritative Special Schedule. A Special-only plan remains valid, while newly created Special plans also pin the applicable normal version.
...
- The Staff schema permits multiple active School Subs, stores nullable positive `standard_period_minutes`, uses globally unique normalized Staff aliases, and indexes candidate configuration. Staff/Room APIs use deactivation rather than deletion. School Sub writes enforce Can Sub, and disabling Can Sub clears School Sub. The Staff & Rooms workspace supplies focused search, add/edit, configuration, aliases, and inactive-record recovery without becoming an HR system.
- `school_calendar_dates` stores durable per-date school-calendar metadata. Calendar records can replace the rotation-derived expected A/B value, but do not select schedules or prevent an administrator from choosing a different A/B value. Non-school, blackout, and expected-Special flags are informational; a missing configured Special Schedule becomes a warning only. The schedule adapter recognizes explicit `Off-site` blocks and the shared availability classifier gives any overlap precedence over inferred open time. A trailing numeric/alphanumeric room token such as `(203A)`, `(P1)`, or `(A-12)` overrides a header/home room; other parenthetical text remains part of the activity description.
- `school_calendar_dates` is the authoritative store for explicit calendar-date configuration; missing dates remain unconfigured and use the existing fallback behavior. Calendar Administration reads bounded persisted rows and writes one date at a time using validated manual-admin upsert/delete operations. Deleting a row returns it to fallback; it is distinct from an explicit No School row. The legacy destructive replacement operation is retained only for compatibility and is not used by Calendar Administration; a future import flow must preview, diff, and confirm date upserts instead of replacing the calendar. Calendar edits preserve existing Daily Sub Plan schedule IDs and finalization state, so finalized historical plans are never rebuilt or rewritten.
- Assignments store a structural shared-responsibility key for absence-created non-instructional responsibilities. The key uses the Daily Sub Plan's pinned schedule source, day applicability, interval, activity/category, normalized description, and room; it never uses a currently active schedule. Sibling records remain both for absence auditability and as the replacement-capacity positions vacated by absent scheduled staff. The operational read model groups them into one Need while exposing each vacant position to the resolver. A replacement is written to one specific sibling; two absent originals therefore permit two distinct replacements. Legacy explicit `solo_coverage` records stay readable, while current plans derive Solo from all present scheduled co-assignees plus direct or split replacement coverage. Exactly one actual staffer is operationally resolved and shown as Solo without consuming an unfilled replacement option; two or more is normally staffed; none is unresolved. The projection creates no workload rows, so scheduled Solo remains workload-exempt and each replacement is counted only by its underlying Assignment.
...
- Generated communication is a deterministic, teacher-grouped projection of the current structured Daily Sub Plan. The Worker stores sanitized constrained HTML alongside a plain-text fallback in `generated_messages`; historical text-only rows are converted to simple paragraphs on read. Direct, split, and shared-duty covering names are bold in HTML while plain text remains markup-free. The client uses a browser-native, constrained contenteditable surface for paragraphs, bullets, bold, paste, and native undo/redo rather than adding a document-editor dependency. Regeneration creates a new projection version and deliberately replaces edited content; it never alters Assignments. Shared-duty siblings are projected once, with the first absent teacher's section owning the operational line and all actual staffers represented on that line.
- `daily_sub_plans.structured_revision` and `generated_messages.source_plan_revision` tie generated communication to structured state. Absence, resolution, split, bulk-resolution, room, and Note mutations increment the plan revision in the same D1 batch as the structured change. Message edits, regeneration, finalization, and reopening do not. A message with a different or historical null source revision is stale. Review regenerates stale unedited content automatically, preserves stale manually edited content with a warning, and requires regeneration before finalization; the Worker independently rejects stale finalization.
- Absence creation rejects overlapping coverage for the same staff member; full-day coverage conflicts with any same-date coverage, while adjacent half-open partial-day intervals remain valid. Absence removal is a Worker-owned atomic operation. Removing one date shortens or deterministically splits the inclusive range, rekeys later-date Assignments to a new Absence when needed, removes generated Needs, increments the affected plan revision, preserves any edited message as stale, and never reconstructs resolutions previously invalidated when the Absence was added. Removing the block deletes all of its generated Assignments. Every affected date is preflighted and any finalized Daily Sub Plan must be reopened first.
- Clearing a resolution is a Worker-owned Assignment transition that removes direct, structured, Not Covered, or split primary state and override acknowledgement while preserving supplemental room and Note context and the Default reference. A current shared-duty replacement clears only its underlying vacant position, so remaining staffing is reprojected and may become inferred Solo; legacy explicit `solo_coverage` records retain group-clear compatibility. Shared-duty summary and Affected Only counts are operational group counts rather than physical Assignment-row counts.

- Production deploy automation is a GitHub Actions workflow limited to trusted `main` pushes or explicit manual dispatch. It runs the same `npm run deploy:production` path as operators, serializes production runs, and uses protected-environment secrets for Cloudflare API credentials plus environment variables for production Access identifiers and a separately configured, non-sensitive health URL because the canonical production hostname is not tracked in Wrangler configuration. The script passes both the Vite `CLOUDFLARE_ENV=production` build selection and Wrangler `--env production` deployment selection, so the generated Worker manifest binds the production D1 configuration. D1 migrations remain a separately reviewed operator action and are never part of an application deploy.

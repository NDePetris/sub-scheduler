...
A candidate is automatically available only when a PLAN or Admin block covers the relevant time, or according to configured School Sub availability. Other activities do not create availability. Existing coverage, absence, and schedule conflicts become explainable warnings or unavailability signals. Administrators may explicitly override warnings.
...
3. PLAN-available staff;
4. Admin-available staff;
5. manually selectable staff.
